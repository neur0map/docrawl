...
pub fn normalize_url(u: &Url) -> String {
    ...
    if (clone.scheme() == "http" && clone.port() == Some(80))
        || (clone.scheme() == "https" && clone.port() == Some(443))
    {
    ...
    clone.set_path(&path);
    // Remove empty query
    if clone.query().map(|q| q.trim().is_empty()).unwrap_or(false) {
...
pub fn path_for_url(output_root: &Path, base: &Url, target: &Url) -> PathBuf {
    ...
    // Determine filename
    let (mut file_stem, ext): (String, &str) =
        if path.ends_with('/') || path.is_empty() || segments.is_empty() {
    ...
    output_root.join(base_host).join(rel).join(file_name)
}
...
pub fn path_for_asset(output_root: &Path, base: &Url, asset: &Url) -> PathBuf {
    ...
    let file_name = if let Some(last) = segments.last() {
        if last.ends_with('/') || last.is_empty() {
            None
        } else {
            Some(last.to_string())
        }
    ...
    rel.extend(segments.into_iter().map(sanitize_segment));
