...
pub fn extract_page(base: &Url, html: &str, preferred_selectors: Option<&[String]>) -> ExtractedPage {
    let doc = Html::parse_document(html);

    let mut title = extract_title(&doc).unwrap_or_else(|| "Untitled".to_string());
    let main_html_raw = extract_main_html(&doc, preferred_selectors).unwrap_or_else(|| html.to_string());
    let main_html_raw = clean_with_soup(&main_html_raw);
    if title.trim().is_empty() { title = "Untitled".to_string(); }
    ...
    ExtractedPage { title, main_html, links, images }
}
...
fn extract_links(base: &Url, doc: &Html) -> Vec<Url> {
    let sel_a = match Selector::parse("a[href]") { Ok(s) => s, Err(_) => return vec![] };
    let mut links = vec![];
    for a in doc.select(&sel_a) {
        if let Some(href) = a.value().attr("href") {
            let href = href.trim();
            if href.is_empty() { continue; }
            if href.starts_with('#') || href.starts_with("mailto:") || href.starts_with("javascript:") || href.starts_with("tel:") { continue; }
...
fn extract_images(base: &Url, doc: &Html) -> Vec<Url> {
    let sel = match Selector::parse("img[src]") { Ok(s) => s, Err(_) => return vec![] };
    let mut out = vec![];
    for img in doc.select(&sel) {
        if let Some(src) = img.value().attr("src") {
            let src = src.trim();
            if src.is_empty() { continue; }
            if let Ok(u) = base.join(src) { out.push(u); }
        }
    }
    out
}

fn rewrite_for_markdown(base: &Url, html: &str, anchor_fallbacks: &HashMap<String, String>) -> String {
...
fn compute_anchor_fallbacks(base: &Url, doc: &Html) -> HashMap<String, String> {
    ...
    let sel_a = match Selector::parse("a[href]") { Ok(s) => s, Err(_) => return map };
    for a in doc.select(&sel_a) {
        let href = match a.value().attr("href") { Some(h) => h.trim(), None => continue };
        if href.is_empty() { continue; }
        let text = a.text().collect::<Vec<_>>().join("").trim().to_string();
        if !text.is_empty() { continue; }
        let abs = match base.join(href) { Ok(u) => u, Err(_) => continue };
